...
class TranscriptEvidenceLine(BaseModel):
    """A single cited line from the transcript with metadata."""
    timestamp: str             # MM:SS
    speaker: str
    quote: str                 # verbatim or near-verbatim text
    relevance: str             # why this line is evidence


class ExplainableEvaluationResult(BaseModel):
    """Explainable audit result — every score justified with evidence."""
    category: str
    category_id: str
    score: float               # 0–10
    why_score: str             # plain-English explanation of this exact score
    reason_for_zero: str = ""  # required when score < 2
    evidence_found: list[str] = Field(default_factory=list)      # what the counsellor DID
    missing_behaviors: list[str] = Field(default_factory=list)   # what was absent
    transcript_evidence: list[TranscriptEvidenceLine] = Field(default_factory=list)
    expected_behaviors: list[str] = Field(default_factory=list)  # what 9-10 looks like
    coaching_feedback: str = ""
    score_confidence: float = 1.0   # 0–1; low when transcript is sparse
    evaluation_basis: str = "gpt"   # "gpt" | "rule" | "na" (not applicable)
...
# ── Participant Intelligence models ──────────────────────────────────────────

class ParticipantProfile(BaseModel):
    speaker_id: str
    role: Speaker
    word_count: int
    word_share_pct: float
    turn_count: int
    questions_asked: int
...
class AttendanceRecord(BaseModel):
    ...
    parent_attendance_pct: float
    meeting_attendance_quality: str   # "full" | "partial" | "student_only"


class ParticipantIntelligence(BaseModel):
    participants: list[ParticipantProfile]
    attendance: AttendanceRecord
    student_engagement_score: float
    parent_engagement_score: float
    counsellor_talk_ratio: float
    stakeholder_coverage_score: float
    participation_quality_score: float
    audit_impact_narrative: str
    flags: list[str]
...
class DemoAuditReport(BaseModel):
    ...
    # Participant Intelligence (optional — populated when diarization detects speakers)
    participant_intelligence: Optional[ParticipantIntelligence] = None
